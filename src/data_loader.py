...
class FAT_ValSet_logmel(Dataset):
    def __init__(self, config, fnames, dataframe, labels, transform=None):
        self.config = config
        self.dataframe = dataframe
        self.labels = labels
        self.transform = transform
    ...
    def __len__(self):
        return len(self.dataframe) * self.tta
    ...
    def __getitem__(self, idx):
        ...
        # Read and Resample the audio
        data = self._random_selection(file_path)
        label = torch.from_numpy(self.labels[new_idx]).float()
        fname = self.fnames[new_idx]
